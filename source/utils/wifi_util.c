    ...
    if ((band == WIFI_FREQUENCY_5_BAND) || (band == WIFI_FREQUENCY_5L_BAND) || (band == WIFI_FREQUENCY_5H_BAND)){
        ...
        if (bandwidth == WIFI_CHANNELBANDWIDTH_40MHZ) {
            ...
            if (found_idx != -1) {
                ...
                wifi_util_info_print(WIFI_WEBCONFIG, "%s:%d: on_channel list is done\n", __func__, __LINE__);
                return 0;
            } else {
                wifi_util_info_print(WIFI_WEBCONFIG, "%s:%d: on_channel list is empty and returned with error\n", __func__, __LINE__);
                return -1;
            }
        }
    }
