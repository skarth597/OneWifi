        ...
        struct schema_Wifi_Global_Config *new_rec)
{
    ...
    else if ((mon->mon_type == OVSDB_UPDATE_NEW) || (mon->mon_type == OVSDB_UPDATE_MODIFY))
    {
        ...
        wifi_util_dbg_print(WIFI_DB,"%s:%d: rss_check_interval%d rss_threshold:%d rss_maxlimit:%d heapwalk_duration:%d heapwalk_interval:%d\n", __func__, __LINE__, new_rec->rss_check_interval, new_rec->rss_threshold, new_rec->rss_maxlimit, new_rec->heapwalk_duration, new_rec->heapwalk_interval);
        wifi_util_dbg_print(WIFI_DB,"%s:%d: memwraptool_enable:%d\n", __func__, __LINE__, new_rec->memwraptool_enable);
        pthread_mutex_unlock(&g_wifidb->data_cache_lock);
    }
    else
    {
